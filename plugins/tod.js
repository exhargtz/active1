...
let handler = async (m, { conn, command }) => {
  if (/^tod$/i.test(command)) {
    conn.send2Button(m.chat, 'Truth or Dare', 'Truth', '.truth', 'Dare', '.dare', 'RANDOM', `${pickRandom([',dare', ',truth'])}`, m)
  }
  if (/^truth$/i.test(command)) {
    ...
    conn.send2Button(m.chat, json.message, '©Nahara', 'Truth', ',truth', 'Dare', '.dare', m)
