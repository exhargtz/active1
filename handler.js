  ...
  async handler(chatUpdate) {
    ...
    ayam: 0,
        ...
        if (chat) {
          ...
          if (!('antiLinkNomor' in chat)) chat.antiLinkNomor = false
      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            let pp = 'https://i.ibb.co/D1wnk2D/20210905-005846.png'
            try {
              ...
              text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat datang, @user!').replace('@subject', this.getName(jid)).replace('@desc', groupMetadata.desc) :
                (chat.sBye || this.bye || conn.bye || '*Byee @user*\n```Makasih Kunjungannya...```')).replace('@user', '@' + user.split`@`[0])
...
`.trim(), '', 'DISABLE THIS', '/enable delete', {
      quoted: m.message,
      contextInfo: {
    ...
    async onCall(json) {
        ...
        await this.sendMessage(from, 'Jangan Menelpon Bot Dapat Menyebabkan Blokir Permanent!!', MessageType.extendedText)
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    unreg: '── 「 TIDAK TERDAFTAR 」 ──\nSilakan Register Terlebih Dahulu Sebelum Menggunakan Bot. Cara Daftar Hanya Ketik :\n\n*#daftar Namamu.umur*\n\n*Cnth : #daftar nahara.16*\nNote : Jangan Spasi Saat di Titik',
    nsfw: ' tidak aktif. Hanya pemilik bot yang bisa mengaktifkannya'
  }[type]
